  ...
  function pageSettings() {
    ...
    var drawDefs = function () {
      ...
      $('#stores').innerHTML = '<h3 style="margin-top:0">店舗</h3><table class="tbl"><thead><tr><th>店舗名</th><th>並び</th><th>解禁日</th><th>解禁時刻</th><th>全体カレンダー</th><th>有効</th><th></th></tr></thead><tbody>' + me.stores.map(function (s) {
        ...
        return '<tr><td>' + esc(s.name) + '</td><td><input type="number" data-f="sort" value="' + s.sort + '" style="width:60px"></td><td><input type="text" data-f="releaseDay" value="' + esc(s.releaseDay === null ? '' : s.releaseDay) + '" style="width:50px" placeholder="共通"></td><td><input type="text" data-f="releaseTime" value="' + esc(s.releaseTime) + '" style="width:70px" placeholder="共通">' + pend + '</td><td>' + (s.hasCalendar ? '作成済' : '<span class="tag yellow">未作成</span>') + '</td><td><input type="checkbox" data-f="active"' + (s.active ? ' checked' : '') + '></td><td class="actions"><button type="button" class="btn small sub ssave" data-name="' + esc(s.name) + '">保存</button></td></tr>';
      }).join('') + '<tr><td><input type="text" id="sname" placeholder="新しい店舗名" style="width:120px"></td><td><input type="number" id="ssort" value="9" style="width:60px"></td><td></td><td></td><td></td><td><input type="checkbox" id="sactive" checked></td><td class="actions"><button type="button" class="btn small" id="sadd">追加</button></td></tr></tbody></table><div class="muted small">解禁日・解禁時刻は、店舗ごとに解禁をずらすときだけ入れます（空欄＝設定の「予約解禁日」「予約解禁時刻」）。変えると次の月の1日から効きます（並び・有効はすぐ）。全体カレンダーは、先生の画面の「カレンダーを作成・共有」で作られます。</div>';
