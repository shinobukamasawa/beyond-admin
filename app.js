...
(function () {
  ...
  function pageSettings() {
    ...
    var drawDefs = function () {
      ...
      $('#stores').innerHTML = '<h3 style="margin-top:0">店舗</h3><table class="tbl"><thead><tr><th>店舗名</th><th>並び</th><th>解禁日</th><th>解禁時刻</th><th>LINE の送信元</th><th>全体カレンダー</th><th>有効</th><th></th></tr></thead><tbody>' + me.stores.map(function (s) {
        var pend = s.pendingFrom ? '<div class="msg warn small" style="margin-top:4px">' + esc((Number(String(s.pendingFrom).substring(5)) || '') + '月1日から「' + (s.pendingReleaseDay === null ? '共通' : s.pendingReleaseDay + '日') + '・' + (s.pendingReleaseTime || '共通') + '」になります') + ' <button type="button" class="btn small sub scancel" data-name="' + esc(s.name) + '">取り消す</button></div>' : '';
        return '<tr><td>' + esc(s.name) + '</td><td><input type="number" data-f="sort" value="' + s.sort + '" style="width:60px"></td><td><input type="text" data-f="releaseDay" value="' + esc(s.releaseDay === null ? '' : s.releaseDay) + '" style="width:50px" placeholder="共通"></td><td><input type="text" data-f="releaseTime" value="' + esc(s.releaseTime) + '" style="width:70px" placeholder="共通">' + pend + '</td><td><input type="text" data-f="lineChannel" value="' + esc(s.lineChannel || '') + '" style="width:110px" placeholder="共通"></td><td>' + (s.hasCalendar ? '作成済' : '<span class="tag yellow">未作成</span>') + '</td><td><input type="checkbox" data-f="active"' + (s.active ? ' checked' : '') + '></td><td class="actions"><button type="button" class="btn small sub ssave" data-name="' + esc(s.name) + '">保存</button></td></tr>';
      }).join('') + '<tr><td><input type="text" id="sname" placeholder="新しい店舗名" style="width:120px"></td><td><input type="number" id="ssort" value="9" style="width:60px"></td><td></td><td></td><td></td><td></td><td><input type="checkbox" id="sactive" checked></td><td class="actions"><button type="button" class="btn small" id="sadd">追加</button></td></tr></tbody></table><div class="muted small">解禁日・解禁時刻は、店舗ごとに解禁をずらすときだけ入れます（空欄＝設定の「予約解禁日」「予約解禁時刻」）。変えると次の月の1日から効きます（並び・有効はすぐ）。「LINE の送信元」は、リマインドをどの公式アカウントから送るかの鍵（例：akishima・international。空欄＝共通）。鍵ごとのトークンは MonoSuke が置きます。全体カレンダーは、先生の画面の「カレンダーを作成・共有」で作られます。</div>';
      ...
      $$('.ssave', v).forEach(function (btn) { btn.onclick = function () {
        var o = rowVals(btn); o.name = btn.dataset.name;
        var s = me.stores.find(function (x) { return x.name === o.name; });
        var changed = s && (String(s.releaseDay === null ? '' : s.releaseDay) !== String(o.releaseDay).trim() || s.releaseTime !== String(o.releaseTime).trim());
        if (!changed) { api('stores.save', o).then(after); return; }
        var show = function (d, t) { return (String(d).trim() === '' ? '共通' : String(d).trim() + '日') + '・' + (String(t).trim() === '' ? '共通' : String(t).trim()); };
        confirmBox('店舗の解禁日・時刻の変更：' + o.name, '「' + show(s.releaseDay === null ? '' : s.releaseDay, s.releaseTime) + '」→「' + show(o.releaseDay, o.releaseTime) + '」\n\nいつから：次の月の1日から。それまでは今の値のままで、今の月の解禁（受付）は変わりません。\n\nこの変更は操作ログに残ります。', '次の月から変える').then(function (yes) { if (yes) api('stores.save', o).then(after); else drawDefs(); });
      }; });
      $$('.scancel', v).forEach(function (btn) { btn.onclick = function () {
        confirmBox('予約した値を取り消す', btn.dataset.name + ' の解禁日・時刻の「次の月から」の変更を取り消し、今の値のままにします。', '取り消す').then(function (yes) { if (!yes) return; var s = me.stores.find(function (x) { return x.name === btn.dataset.name; }); api('stores.save', { name: s.name, sort: s.sort, active: s.active, releaseDay: s.releaseDay === null ? '' : s.releaseDay, releaseTime: s.releaseTime, cancelPending: true }).then(after); });
      }; });
